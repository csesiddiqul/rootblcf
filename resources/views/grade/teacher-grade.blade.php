                        ...
                        <div class="panel-body">
                            @foreach ($grades as $grade)
                                <h4>
                                    ...
                                    @if(count($grades) > 0)
                                        @foreach ($grades as $grade)
                                            <b>@lang('Teacher Code')</b> - {{$grade->teacher->student_code}} &nbsp;
                                            <b>@lang('Name')</b> - {{$grade->teacher->name}}
                                            @break($loop->first)
                                        @endforeach
                                    @else
                                ...
                                <div class="row" style="margin-bottom: 2%;">
                                    <div class="col-md-10">
                                        <b>@lang('Course') - </b> <small>{{$grade->course_config->course->name}}</small>
                                        <b>@lang('Class') - </b>
                                        <small>{{$grade->student->section->class->name}}</small>
                                        <b>@lang('Section') - </b>
                                        <small>{{$grade->student->section->section_number}}</small>
                                    ...
                                    @if($grade->course_config->quiz_count > 0)
                                        <div class="col-md-4">
                                            <div class="alert alert-info" role="alert">
                                                <ul>
                                                    <li>@lang('Quiz Counted Best') <span
                                                                class="label label-success">{{$grade->course_config->quiz_count}}</span>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    @endif
                                    @if($grade->course_config->assignment_count > 0)
                                        <div class="col-md-4">
                                            <div class="alert alert-info" role="alert">
                                                <ul>
                                                    <li>@lang('Assignment Counted Best ')<span
                                                                class="label label-success">{{$grade->course_config->assignment_count}}</span>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
                                    @endif
                                    @if($grade->course_config->ct_count > 0)
                                        <div class="col-md-4">
                                            <div class="alert alert-info" role="alert">
                                                <ul>
                                                    <li>@lang('Class Test Counted Best') <span
                                                                class="label label-success">{{$grade->course_config->ct_count}}</span>
                                                    </li>
                                                </ul>
                                            </div>
                                        </div>
