...
<div class="panel-body pad-bot-top">
    <div class="btn-group new_b pull-left" style="overflow: hidden;"> 
        ...
        --}}
        <a href="{{route('academic.category.index')}}"  class="btn {{(\Route::current()->getName() == 'academic.category.index')? 'active':''}}">@lang('Student Categories')</a>
         <a href="{{route('academic.house.index')}}"  class="btn {{(\Route::current()->getName() == 'academic.house.index')? 'active':''}}">@lang('Student House')</a>
     ...
     <div class="pull-right">
        @if(isset($board_exams))
                    <a href="{{route('academic.board_exam.create')}}" class="btn btn-sm foqas-btn pull-left">@lang('Add Board Exam')</a>
        @endif
    </div>
</div>
