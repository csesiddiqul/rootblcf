<div class="row">
    <div class="col-md-2 pad-left-0">
        @if(!empty($user->pic_path))
            <img src="{{asset('img/proloading.gif')}}" data-src="{{url($user->pic_path)}}" class="img-thumbnail" id="my-profile" alt="Profile Picture" width="100%">
            <div class="btn-group removeUpImage" onclick="img_confirm_delete('users','{{$user->id}}','pic_path','your Profile Photo')">
            ...
            @if(\Auth::user()->role == 'admin' || \Auth::user()->id == $user->id)
                <div class="image-upload">
                    <label for="file-upload">
                        <img src="{{asset('img/profile.png')}}" id="preview_image" class="img-responsive">
                    </label>    
    ...
    <div class="col-md-10" id="main-container">
        <h3 class="mg-top-0">{{$user->name}} <span class="label label-danger">{{ucfirst($user->role)}}</span> <span
                    class="label label-primary">{{ucfirst($user->gender)}}</span>
            @if ($user->role == 'teacher' && $user->section_id > 0)
                <small>@lang('Class Teacher of Section'): <span
                            class="label label-info">{{ucfirst($user->section->section_number)}}</span></small>
            @endif
            ...
            @if($user->role == "student")
                @if(Auth::user()->role == 'student' || Auth::user()->role == 'teacher' || Auth::user()->role == 'admin')
                    <a class="btn btn-xs btn-info" role="button"
                       href="{{url('attendances/0/'.$user->id.'/0')}}">@lang('View Attendance')</a>
                    {{--@if (!Session::has('section-attendance'))
                    <td><small><a class="btn btn-xs btn-success" role="button" href="{{route('grade.each_student',$user->student_code)}}">@lang('View Marks')</a></small></td>
                    @endif --}}
                @endif
                <button class="btn btn-xs btn-success pull-right" role="button" id="btnPrint"><i class="material-icons">print</i> @lang('Print Profile')
                </button>
                <div class="visible-print-block" id="profile-content">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="col-xs-4">
                                ...
                                <div style="font-size: 10px;">@lang('Printing Date')
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="col-xs-3">
                                @if(!empty($user->pic_path))
                                    <img src="{{asset('01-progress.gif')}}" data-src="{{url($user->pic_path)}}"
                                         class="img-thumbnail" id="my-profile" alt="@lang('Profile Picture')"
                                         width="120px" height="120px">
                                @else
                                    @if(strtolower($user->gender) == trans('male'))
                                        <img src="{{asset('01-progress.gif')}}"
                                             data-src="https://img.icons8.com/color/48/000000/architect.png"
                                             class="img-thumbnail" id="my-profile" alt="@lang('Profile Picture')"
                                             width="120px" height="120px">
                                    @else
                                        <img src="{{asset('01-progress.gif')}}"
                                             data-src="https://img.icons8.com/color/48/000000/architect-female.png"
                                             class="img-thumbnail" id="my-profile" alt="@lang('Profile Picture')"
                                             width="120px" height="120px">
                                    @endif
                                @endif
