    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-10" id="main-container">
                @include('components.pages-bar',['pageTitle' =>'<a href="'. url('users/'.Auth::user()->school->code.'/accountant').'">'. trans('Manage Accounts').'</a> / <b>'.trans('Income').'<b>'])
                @include('components.sectionbar.accounts-bar')
                <div class="panel panel-default ptlb-515">
                    <div class="panel-body plt-07">
                        <div class="page-panel-title">@lang('List of Income')
                        ...
                        <div class="row">
                            <div class="pull-right">
                                <form id="registerForm" action="{{route('accounts.income.index')}}" method="put"
                                      autocomplete="off">
                                    @csrf
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-data-div table-hover">
                                            ...
                                            <tr>
                                                ...
                                                <th scope="col">@lang('Income Head')</th>
                                            ...
                                            <tbody>
                                            @foreach($income as $incomd)
                                                <tr title="{{$incomd->description}}" class="popTop">
                                                    <td>{{($loop->index + 1)}}</td>
                                                    <td>{{date('d-m-Y',strtotime($incomd->date))}}</td>
                                                    <td>{{$incomd->accountSector->name}}</td>
                                                    <td>{{$incomd->name}}</td>
                                                    <td><a href="{{route('accounts.income.voucher',$incomd->voucher_no)}}">{{$incomd->voucher_no}}</a></td>
                                                    <td>{{number_format($incomd->amount,2)}}</td>
                                                    <td>{{$incomd->invoice_number}}</td>
                                                    <td class="text-center">
                                                        @isset($incomd->file)
                                                            <a href="{{url('/'.$incomd->file)}}" class="btn btn-xs foqas-btn "
                                                               target="_blank">
                                                                <i class="fa fa-eye"></i>
                                                            </a>
                                                        @else
...
{{--                                                    @endif--}}
                                        ...
                                        <p id="mymr" class="font_10 special-box-develop lucida"
                                            style="text-align: center; margin-bottom:8px !important; margin-top:40px; width:100%; font-size: 25px; text-transform: uppercase; border: 1px solid #e9e4e4; "><span class="lucida" style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; font-weight:bold;">@lang('Income List')</span><br>
                                        </p>

                                            <h4 style="text-align:center; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">@lang('Income Year - '.$year)</h4>
                                            <table class="table table-bordered"
                                                ...
                                                <tr style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">
                                                    ...
                                                    <th style="border: 1px solid #888888; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif;">@lang('Income Head')</th>
                                                ...
                                                @foreach($income as $incomd)
                                                    <tr>
                                                        ...
                                                        <td style="border: 1px solid #888888;">{{date('d-m-Y',strtotime($incomd->date))}}</td>
                                                        <td style="border: 1px solid #888888;">{{$incomd->accountSector->name}}</td>
                                                        <td style="border: 1px solid #888888;">{{$incomd->name}}</td>
                                                        <td style="border: 1px solid #888888;">{{$incomd->voucher_no}}</td>
                                                        <td style="border: 1px solid #888888;">{{number_format($incomd->amount,2)}}</td>
                                                        <td style="border: 1px solid #888888;">{{$incomd->invoice_number}}</td>
                                                        <td style="border: 1px solid #888888;">{{Carbon\Carbon::parse($incomd->created_at)->format('Y')}}</td>
                                                    </tr>
                                                @endforeach
                                                </tbody>
                                            </table>
        ...
        @push('script')
            ...
            <script>
                ...
                var config = {
                    ...
                    data: {
                        datasets: [{
                            ...
                            fill: false,
                            data: [@foreach($income->groupBy('date')->sort() as $date => $ex)
                            {
                ...
                options: {
                        title: {
                            display: true,
                            text: @json( __('Income (In SGD) in Time Scale'))
                        },
