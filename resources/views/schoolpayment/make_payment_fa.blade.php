            ...
            <div class="col-md-10" id="main-container">
                <div class="panel panel-default pt-0">
                    ...
                    <div class="panel-body p-0">
                        <div class="col-md-12">
                            <h5 class="">
                                <i><b>@lang('Service charge has been paid till')
                                        <code>
                                            @if(!empty($schools->lastCharged))
                                                {{date('F, Y', strtotime($schools->lastCharged))}}
                                            @else
                                                &#9745; @lang('Service charge not paid yet').
                                            @endif
                                        </code>
                                    </b></i>
                            </h5>
                            <div class="col-md-6 col-sm-6 box_detail">
                                ...
                                <div class="p_radio pr4">Total Service Charge: {{$cny}}<span>{{$total_chage}}</span>
                                </div>
                            ...
                            <div class="col-md-6 col-sm-6">
                                @if($country=='BD' && $amount >= 10)
                                    ...
                                    <input name="month_of" type="hidden" value="{{$months}}" id="month_of" class="month_of" required>
                                ...
                                @if(auth()->user()->role == 'master')
                                    {!! Form::open(array('route' =>['school.services.charge_cash',$schools->code],'method' =>'POST','role' =>'form','autocomplete'=>'off','class'=>'stripe','id'=>'confirm_form_charge_cash')) !!}
                                    <input name="month_of" type="hidden" value="{{$months}}" id="month_of" class="month_of" required>
                                    <button class="plan-detail" onclick="confirmStatus('charge_cash',' collect cash payment amount {{number_format($amount) .' '. ($country=='BD' ? 'Tk':'$')}}')" type="button">@lang('Cash Payment')</button>
                                    {!! Form::close() !!}
                                @endif
                            </div>
                            <div class="clearhight"></div>
