<a class="btn btn-xs btn-info pull-right" data-toggle="collapse" href="#collapseForNewCourse{{$section->id}}" aria-expanded="false" aria-controls="collapseForNewCourse{{$section->id}}">+{{school('country')->code == 'BD' ? 'Add New Subject ' : ' Add New Course'}}</a>
      ...
      <form class="form-horizontal" action="{{url('courses/store')}}" method="post">
              ...
              <select class="form-control" id="teacherDepartment{{$section->id}}" name="teacher_department">
                ...
                @if(count($departments) > 0)
                  ...
                  @foreach ($departments_of_this_school as $d)
                    <option value="{{$d->department_name}}">{{translate()->setSource('en')->setTarget(session('localLang') ?? 'en')->translate($d->department_name)}}</option>
                  @endforeach
                @endif
              </select>
            </div>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
            ...
            <span id="helpBlock" class="help-block">@lang('Ex: 12:50PM-01:40PM Sunday')</span>
          </div>
        </div>
        <div class="form-group">
