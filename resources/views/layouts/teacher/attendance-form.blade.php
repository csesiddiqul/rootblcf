...
<form action="{{url('attendance/take-attendance')}}" method="post">
                ...
                @foreach ($attendances as $attendance)
                    <tr>
                            ...
                            @if($attendance->present === 1)
                                <div class="form-check">
                                    <input class="form-check-input position-static" type="checkbox" aria-label="Present"
                                           name="isPresent{{$loop->index}}" checked>
                                </div>
                            @else
                                <div class="form-check">
                                    <input class="form-check-input position-static" type="checkbox"
                                           name="isPresent{{$loop->index}}" aria-label="Absent">
                                </div>
                            @endif
                        </td>
                        @if(count($attCount) > 0)
                        ...
                        @else
                            ...
                            <td>0</td>
                        @endif
                        <td><a href="{{route('attendance.adjust',[$attendance->student->id,$course_id,$exam_id])}}"
                               role="button" class="btn btn-danger btn-sm">@lang('Adjust Missing Attendances')</a></td>
                    </tr>
                @endforeach
            @else
                ...
                @foreach ($students as $student)
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="form-check">
                                <input class="form-check-input position-static" type="checkbox"
                                       name="isPresent{{$loop->index}}" aria-label="Present" checked>
                            </div>
                        </td>
                        @if(count($attCount) > 0)
                            @foreach ($attCount as $at)
                        ...
                        @else
                            ...
                            <td>0</td>
                        @endif
                        <td><a href="{{route('attendance.adjust',[$student->id,$course_id,$exam_id])}}" role="button"
                               class="btn btn-danger btn-sm">@lang('Adjust Missing Attendances')</a></td>
                    </tr>
                @endforeach
            @endif
            </tbody>
    ...
    <div align="center">
        <div class="col-md-2 pl-0">
            <a href="javascript:history.back()" class="btn btn-danger btn-sm btn-block" style="margin-right: 2%;"
